#supple. fig.5-> umap scTv2. features 1000,2000, 4000

#source("~/3.scripts/main_figures/supple_fig.5.R")

#generate_supple_fig.5(dataName.list="facs10", filter_rate=0)

# Top4(HDG, HEG, HIPPO, NBDrop)
# Low4(HVG.disp, HVG.vst, NBDisp, M3Drop)

generate_supple_fig.5 <- function(dataName.list, filter_rate){
  ####### data loading ################################
  ...
  features.list = c(1000, 2000, 4000)
  N.f = length(features.list)
  # top umaps
  for (f in 1:N.f){
    method.list.top4 = c("scTv2.HDG", "scTv2.HEG.cpm",  "scTv2.HIPPO", "scTv2.NBDrop")
    pTop.list = generate_integrated_ARI_EGR.violin_umap_scTv2_top4(nfeatures=features.list[f], obj=obj.org, real.subtype, cpm.data.org, method.list=method.list.top4, width=1)
    p.list = pTop.list$p.umap.list
    
    x= length(p.list)
    for(i in 1:x){
      pName = paste("pT",f, i,sep="")
      assign(pName, p.list[[i]])
    }
  }
  
  # bottom umaps
  for (f in 1:N.f){
    method.list.bottom4 = c("scTv2.HVG.disp", "scTv2.HVG.vst",  "scTv2.NBDisp", "scTv2.N3Drop")
    pBottom.list = generate_integrated_ARI_EGR.violin_umap_scTv2_bottom4(nfeatures=features.list[f], obj=obj.org, real.subtype, cpm.data.org, method.list=method.list.bottom4, width=0.1)
    p.list = pBottom.list$p.umap.list
    
    x= length(p.list)
    for(i in 1:x){
      pName = paste("pB",f, i,sep="")
      assign(pName, p.list[[i]])
    }
  }
  ...
  plotName = paste("Supple_Fig.5__Top4_and_Bottom4_",dataName, "_", filter_rate, "_",timestamp,".pdf", sep="")
  
  pdfName = paste(file.path.summary, plotName, sep="")
  
  default.txt = paste("Supple Fig.5: Top 4 and Bottom 4 feature selections - ",  dataName, sep="")
  ...
  # p.umap.legend<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.HDG, real.subtype = real.subtype, method="HDG", nfeatures = 500, dist="poisson", LPos="bottom")
  # pdf("legend_umap_facs10.3.pdf", 20,10)
  # p.umap.legend
  # dev.off()
}
...
generate_integrated_ARI_EGR.violin_umap_scTv2_top4 <-function(nfeatures, obj, real.subtype, cpm.data.org, method.list, width){
  library(Seurat)
  library(dplyr)
  library(patchwork)
  library(sctransform)
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.scTv2.HDG, real.subtype = real.subtype, method="HDG", nfeatures =nfeatures, dist="poisson")
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.HEG.cpm, real.subtype = real.subtype, method="HEG.cpm", nfeatures =nfeatures, dist=NULL)
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.HIPPO, real.subtype = real.subtype, method="HIPPO", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.NBDrop, real.subtype = real.subtype, method="NBDrop", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
...
generate_integrated_ARI_EGR.violin_umap_scTv2_bottom4 <-function(nfeatures, obj, real.subtype, cpm.data.org, method.list, width){
  library(Seurat)
  library(dplyr)
  library(patchwork)
  library(sctransform)
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.scTv2.HVG.disp, real.subtype = real.subtype, method="HVG.disp", nfeatures = nfeatures, dist=NULL, LPos="none")
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.HVG.vst, real.subtype = real.subtype, method="HVG.vst", nfeatures = nfeatures, dist=NULL, LPos="none")
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.NBDisp, real.subtype = real.subtype, method="NBDisp", nfeatures = nfeatures, dist=NULL, LPos="none")
  p.umap.list[[j]] = p
  ...
  # p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.M3Drop, real.subtype = real.subtype, method="M3Drop", nfeatures = nfeatures, dist=NULL, LPos="none")
  p.umap.list[[j]] = p
...
generate_ARI.plot_fig3 <- function(dataName.list = c("Zheng_sorted"), models.list = c("scTv2")){
  library("ggplot2")
  library(RColorBrewer)
  library(scales)
...
generate_ASW.plot <- function(dataName.list = c("Zheng_sorted"), models.list = c("scTv2")){
  library("ggplot2")
  library(RColorBrewer)
  library(scales)
