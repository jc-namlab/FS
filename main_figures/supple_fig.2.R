#suppl.fig.2
# : (alz scd )- scTv2 umap, features 1000, 2000, 4000

# Top4(HDG, HEG, HIPPO, NBDrop)
# Low4(HVG.disp, HVG.vst, NBDisp, M3Drop)

#source("~/3.scripts/main_figures/supple_fig.2.R")

#generate_supple_fig.2(dataName="alz_scd_v3", filter_rate=0)


generate_spple_fig.2 <- function(dataName, filter_rate){
  ####### data loading ################################
  ...
  for (f in 1:N.f){
    ...
    pTop.list = generate_integrated_EGR.violin_umap_top4(nfeatures=features.list[f], obj=obj.org, real.subtype, cpm.data.org, method.list = method.list.top4, width=1)
  ...
  for (f in 1:N.f){
    ...
    pBottom.list = generate_integrated_EGR.violin_umap_bottom4(nfeatures=features.list[f], obj=obj.org, real.subtype, cpm.data.org, method.list=method.list.bottom4,width=0.1)
  ...
  plotName = paste("Supple_Fig.2__Top4_and_Bottom4_",dataName, "_", filter_rate, "_",timestamp,".pdf", sep="")
  
  pdfName = paste(file.path.summary, plotName, sep="")
  
  default.txt = paste("Supple Fig.2: Top 4 and Bottom 4 feature selections - ",  dataName, sep="")
  ...
  # pdf("legend_umap_alz_scd_v3_fig.2.pdf", 20,10)
  # p.umap.legend
  # dev.off()
...
generate_integrated_EGR.violin_umap_top4 <-function(nfeatures, obj, real.subtype, cpm.data.org, method.list, width){
  library(Seurat)
  library(dplyr)
  library(patchwork)
  library(sctransform)
...
generate_integrated_EGR.violin_umap_bottom4 <-function(nfeatures, obj, real.subtype, cpm.data.org, method.list, width){
  library(Seurat)
  library(dplyr)
  library(patchwork)
  library(sctransform)
  ...
  #p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.scTv2.HVG.disp, real.subtype = real.subtype, method="HVG.disp", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
  ...
  #p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.HVG.vst, real.subtype = real.subtype, method="HVG.vst", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
  ...
  #p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.NBDisp, real.subtype = real.subtype, method="NBDisp", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
  ...
  #p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.scTv2.M3Drop, real.subtype = real.subtype, method="M3Drop", nfeatures = nfeatures, dist=NULL)
  p.umap.list[[j]] = p
...
generate_ASW.plot <- function(dataName.list = c("alz_scd_v3"), models.list = c("scTv2")){
  library("ggplot2")
  library(RColorBrewer)
  library(scales)
