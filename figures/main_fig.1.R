#fig1. (DiHiData- alz scd) ARI,DEG * scTv2, logN 
#-> (suppl. scd lung, lcd alz, lcd, lung)

#source("~/3.scripts/main_figures/main_fig.1.R")
#generate_main_fig.1(runName = "DrHiDepth_featureNumbers.hvg.norm", dataName.list= "alz_scd_v3")

generate_main_fig.1 <- function( runName = "DrHiDepth_featureNumbers", dataName.list= "alz_scd_v3"){
  library("ggplot2")
  library(RColorBrewer)
  library(scales)
  
  dir.result = "tmp.sim.DrHiDepth_featureNumbers.logNfiltering"
  runName="DrHiDepth_featureNumbers.logNfiltering"
  
  # dir.result = "tmp.sim.DrHiDepth_featureNumbers.hvg.norm"
  # runName="DrHiDepth_featureNumbers.hvg.norm"
  ...
  for(v in 1:N.v){
    ...
    for(n in 1:N.data){
      ...
      for(f in 1:N.f){
        ...
        plotName = paste("TEST.logNfiltering__Main_Fig.1__FeatureSelectionTrend_in_ARI_&_DEG","_DrHiDepthSimData_", dataName.list[n], "_", filter_rate, "_",timestamp,".pdf", sep="")
        
        #plotName = paste("TEST.HVG.norm__Main_Fig.1__FeatureSelectionTrend_in_ARI_&_DEG","_DrHiDepthSimData_", dataName.list[n], "_", filter_rate, "_",timestamp,".pdf", sep="")
        # plotName = paste("Main_Fig.1__FeatureSelectionTrend_in_ARI_&_DEG","_DrHiDepthSimData_", dataName.list[n], "_", filter_rate, "_",timestamp,".pdf", sep="")
        
        pdfName = paste(file.path.summary, plotName, sep="")
        default.txt = paste("TEST.logN.filtering: Feature Selection Trend in ARI & DEG",  "\n",
                            "DrHiSimData_", dataName.list[n], "HighlyExp", 100*(1-filter_rate),"%", sep="")

        # default.txt = paste("TEST.HVG.norm: Feature Selection Trend in ARI & DEG",  "\n",
        #                     "DrHiSimData_", dataName.list[n], "HighlyExp", 100*(1-filter_rate),"%", sep="")
