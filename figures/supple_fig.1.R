#suppl.fig1. logN version
#: (alz scd) Top4 (EGR(violin), umapf500), Bottom4 (EGR(violin), umapf500) 4*4 plot 

# logN
# Top4(DUBStepR, HDG, HIPPO, HEG)
# Low4(HVG.disp, HVG.vst, M3Drop, NBDisp)
#
# scTv2
# Top4(HDG, HEG, HIPPO, NBDrop)
# Low4(HVG.disp, HVG.vst, NBDisp, M3Drop)

#source("~/3.scripts/main_figures/supple_fig.1.R")

#generate_supple_fig.1(dataName="alz_scd_v3", filter_rate=0)


generate_supple_fig.1 <- function(dataName, filter_rate){
  ####### data loading ################################
  ...
  method.list.top4 = c("logN.DUBStepR","logN.HDG",  "logN.HIPPO", "logN.HEG.cpm")
  ...
  method.list.bottom4 = c("logN.HVG.disp", "logN.HVG.vst",  "logN.M3Drop","logN.NBDisp")
  ...
  p.asw <- generate_ASW.plot(dataName.list= c("alz_scd_v3"), models.list=c("logN"))
  ...
  plotName = paste("Supple_Fig.1__Top4_and_Bottom4_",dataName, "_", filter_rate, "_",timestamp,".pdf", sep="")
  
  pdfName = paste(file.path.summary, plotName, sep="")
  
  default.txt = paste("Supple Fig.1: Top 4 and Bottom 4 feature selections - ",  dataName, sep="")
...
generate_integrated_EGR.violin_umap_top4 <-function(obj, real.subtype, cpm.data.org, method.list, width){
  ...
  i=1; j=1
  p <- generate_EGR.violin.plot(model.name= method.list[j], cpm.data.org, obj.name= obj.logN.DUBStepR, method="DUBStepR", dist=NULL, width=width)
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.logN.DUBStepR, real.subtype = real.subtype, method="DUBStepR", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  print("method top1.logN.DUBStepR...ARI Done!")
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.HDG, real.subtype = real.subtype, method="HDG", nfeatures = 500, dist="poisson")
  
  p.umap.list[[j]] = p
  print("method top2.logN.HDG...ARI Done!")
  
  #17. "logN.HIPPO"
  obj.logN.NBDrop = obj.logN.HVG.vst
  i=1; j=3
  p <- generate_EGR.violin.plot(model.name= method.list[j], cpm.data.org, obj.name= obj.logN.NBDrop, method="HIPPO", dist=NULL, width=width)
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.NBDrop, real.subtype = real.subtype, method="HIPPO", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.HEG.cpm, real.subtype = real.subtype, method="HEG.cpm", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  print("method top 4.logN.HEG.cpm...ARI Done!")
  #######################################
...
generate_integrated_EGR.violin_umap_bottom4 <-function(obj, real.subtype, cpm.data.org, method.list, width){
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.logN.HVG.disp, real.subtype = real.subtype, method="HVG.disp", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.HVG.vst, real.subtype = real.subtype, method="HVG.vst", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.M3Drop, real.subtype = real.subtype, method="M3Drop", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  print("method bottom 3.logN.M3Drop...ARI Done!")
  ...
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j],obj.name = obj.logN.NBDisp, real.subtype = real.subtype, method="NBDisp", nfeatures = 500, dist=NULL)
  p.umap.list[[j]] = p
  print("method bottom 4.logN.NBDisp..ARI Done!")
    #######################################
...
generate_UMAP.realSubtype_obj_features <- function(model.name, obj.name, real.subtype, method, nfeatures, dist=NULL){
  ...
  p.umap <- DimPlot(obj, reduction ="umap", pt.size=0.1, group.by="real_subtypes", cols=mycols) + 
    ...
    theme(axis.text=element_text(angle=45, hjust=1, vjust=1, color="black", size=14),
          ...
          legend.position="none")  # #theme(legend.position="bottom")
...
featureSelection <- function(obj.name, method, nfeatures = 2000, dist=NULL){
  ...
  if(method == "DUBStepR"){
    ...
    dataName = "alz_scd_v3"
    model.name = "logN"
    file.dir="~/8.data/allFeatures_DUBStepR_filter0.05/"
...
generate_ASW.plot <- function(dataName.list = c("alz_scd_v3"), models.list = c("logN")){
  library("ggplot2")
  library(RColorBrewer)
  library(scales)
