# sfig.10.a.b.c.d (DiHiData- lung scd) ARI,DEG * scTv2, logN- filter 10% 20% 50% 70%

#source("~/3.scripts/main_figures/supple_fig.10.R")
#generate_supple_fig.10(runName = "DrHiDepth_featureNumbers.hvg.norm", dataName.list= "lung_myeloid_scd_v3")

generate_supple_fig.10 <- function( runName = "DrHiDepth_featureNumbers", dataName.list= "lung_myeloid_scd_v3"){
  library("ggplot2")
  ...
  for(v in 1:N.v){
    ...
    dataName.list =  "lung_myeloid_scd_v3"
    ...
    for(n in 1:N.data){
      ...
      for(f in 1:N.f){
          ...
          for(s in 1:2){
            ...
            if(s==1){
              ...
              p <- ggplot(plot.data,  aes(x=nfeatures, y=ARI, color=method, group=method, linetype=method, size=method)) + 
                geom_point(col=3) + 
                geom_line() + coord_cartesian(ylim = c(0, 0.91)) +
            ...
            if(s==2){ 
              ...
              for(d in 1:N.deg){
                ...
                p <- ggplot(plot.data,  aes(x=nfeatures, y=trueDEGR, color=method, group=method, linetype=method, size=method)) + 
                  #geom_point(col=3) + 
                  geom_line() + coord_cartesian(ylim = c(0.47, 0.81)) +
                  ggtitle(ptitle) +
        ...
        plotName = paste("Supple_Fig.10",letters[f],"__FeatureSelectionTrend_in_ARI_&_DEG","_DrHiDepthSimData_", dataName.list[n], "_", "Hexp",filter.txt,"pro" ,"_",timestamp,".pdf", sep="")
