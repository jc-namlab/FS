#fig3. (facs10) top4(ARI,EGR umap * scTv2, logN) -> top4(3*2 plot) + bottom4 (3*2 plot)in 1 plot 

#source("~/3.scripts/main_figures/fig.3_DUBStepR_cpm_level_plot.R")

#generate_main_fig.3.dubstepR(dataName.list="facs10", filter_rate=0)

# Top4(HDG, HEG, HIPPO, NBDrop)
# Low4(HVG.disp, HVG.vst, NBDisp, M3Drop)

generate_main_fig.3.dubstepR <- function(dataName.list, filter_rate){
  ####### data loading ################################
  ...
  p.ari<- generate_ARI.plot_fig3(dataName.list = c("Zheng_sorted"), models.list = c("scTv2"))
  
  # for ASW plot of all methods
  p.asw <- generate_ASW.plot(dataName.list= c("Zheng_sorted"), models.list=c("scTv2"))
  ...
  N.m= 1#length(models.list)
  for(m in 1:N.m){
    if(m == 1){
      method.list.top4 = c("scTv2.DUBStepR")
  ...
  for(m in 1:N.m){
    ...
    # if(m == 2){
    #   method.list.bottom4 = c("logN.HVG.disp", "logN.HVG.vst",  "logN.NBDisp", "logN.N3Drop")
  ...
  gl.all = list(p.ari,  p.asw, p.n2s,              
              pT211, pT212,pT213,pT214, 
              pT311, pT312,pT313,pT314,
              pB211, pB212,pB213,pB214,
              pB311, pB312,pB313,pB314)
...
generate_all_pdf <- function(gl, dataName, filter_rate, w=30, h=30){
  #******* summary.result plot
  # integrated EGR plot
  ...
  plotName = paste("Main_fig.3__Top4_and_Bottom4_",dataName, "_", filter_rate, "_",timestamp,".pdf", sep="")
  
  pdfName = paste(file.path.summary, plotName, sep="")
  
  default.txt = paste("Fig.3: Top4 and Bottom4 ", "feature selections in ",  dataName, sep="")
...
generate_integrated_ARI_EGR.violin_umap_scTv2_top4 <-function(obj, real.subtype, cpm.data.org, method.list, width){
  ...
  i=1; j=1
  p <- generate_EGR.violin.plot(model.name= method.list[j], cpm.data.org, obj.name= obj.scTv2.DUBStepR, method="DUBStepR", dist=NULL,width=width)
  p.EGR.list[[j]] = p
  p<- generate_UMAP.realSubtype_obj_features(model.name = method.list[j], obj.name = obj.scTv2.DUBStepR, real.subtype = real.subtype, method="DUBStepR", nfeatures = 500, dist=NULL)
  ...
  # #11. scT v2 DUBStepR
  # obj.scTv2.DUBStepR = obj.scTv2.HVG.vst
  # obj.scTv2.DUBStepR <- generate_clustering_results.Vresol.obj_features_DUBStepR(dataName, model.name="scTv2", obj.name = obj.scTv2.DUBStepR, subtype = real.subtype,  method="DUBStepR", nfeatures = nfeatures, dist=NULL, filterN="filter0.05")
...
generate_UMAP.realSubtype_obj_features <- function(model.name, obj.name, real.subtype, method, nfeatures, dist=NULL, LPos="none"){
  ...
  # p.umap <- DimPlot(obj, reduction ="umap", pt.size=0.1) + 
  #   ggtitle(paste(model.name, "DUBStepR nfeatures:", nfeatures,"\n","ARI:",ARI ,"ASW:",ASW)) +
  #   theme(axis.text=element_text(angle=45, hjust=1, vjust=1, color="black", size=14),
  #         axis.title = element_text(size=15, face="bold"),
  #         plot.title = element_text(size=15, hjust=0.5, face="bold"))
  
  # pdf("~/7.results/summary.plot/test_DUBStepR_ASW.pdf", width=20, height=10)
  # library(gridExtra)
  # library(grid)
  # title.txt = "TEST DUBStepR nfeature 1000 vs 2000"
  # grid.arrange(p.umap.1000, p.umap.2000,
  #              top = textGrob(title.txt, gp = gpar(fontsize = 30)), #face = "bold"
  #              nrow = 1)
  # dev.off()
...
featureSelection <- function(obj.name, method, nfeatures = 2000, dist=NULL){
  ...
  if(method == "DUBStepR"){
    ...
    dataName = "Zheng_sorted"
    model.name = "scTv2"
   # "~/8.data/allFeatures_DUBStepR/Features.ordered.DUBStepR.alz_scd_v3.scTv2.nF500.txt"
...
generate_EGR.violin.plot <- function(model.name, cpm.data.org, obj.name, method, dist, width=1){
  ...
  p.dubstepr <- ggplot(df, aes(x=nFeatures, y=cpm.exp.levels, fill=nFeatures)) +
    scale_fill_grey() + 
    geom_violin(width=width) + coord_cartesian(ylim = c(0, N.ylim)) + 
    ggtitle(paste(model.name, " - DUBStepR in facs10 data")) + theme_bw() + 
  ...
  pdf("~/7.results/summary.plot/test_constantARI_DUBStepR.ARI_facs10.pdf", width=20, height=10)
  p.dubstepr
  dev.off()
