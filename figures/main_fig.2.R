#fig2. (alz scd) Top4 (EGR(violin), umapf500), Bottom4 (EGR(violin), umapf500) 4*4 plot 
# ->suppl. (f>500) 
# -> suppl.(alz lung)
...
#source("~/3.scripts/main_figures/main_fig.2.R")

#generate_main_fig.2(dataName="alz_scd_v3", filter_rate=0)


generate_main_fig.2 <- function(dataName, filter_rate){
  ####### data loading ################################
  ...
  plotName = paste("Main_Fig.2__Top4_and_Bottom4_",dataName, "_", filter_rate, "_",timestamp,".pdf", sep="")
  
  pdfName = paste(file.path.summary, plotName, sep="")
  
  default.txt = paste("Fig.2: Top 4 and Bottom 4 feature selections - ",  dataName, sep="")
...
generate_UMAP.realSubtype_obj_features <- function(model.name, obj.name, real.subtype, method, nfeatures, dist=NULL, LPos="none"){
  ...
  if(model.name=="logN"){ds=TRUE}else{ds=FALSE}
  obj <- ScaleData(obj, features = NULL, verbose =FALSE, do.scale=ds)
