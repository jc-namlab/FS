# comp_ARI.ASW.N2S.UMAP_FACS10Data.R
# Juok Cho
# 2021 11 01


comp_ARI.ASW.N2S.UMAP_FACS10Data = function(dataName, count.data, real.subtype, filter_rate){
  ...
  # # QC & filter cells
  # obj.filter <- subset(obj.org, subset = nFeature_RNA > 200 & percent.mt < 5)
  # message("obj after QC & filter cells - ^MT-...")
  # obj.filter
  
  #filt.data <- filt.data[!grepl(pattern = "^MT-|^ERCC-|^RPS|^RPL", x = rownames(filt.data)), ]
  
  # w.MT <- which(grepl(pattern = "^MT-", x = rownames(obj.org)))
  # w.ERCC <- which(grepl(pattern = "^ERCC-", x = rownames(obj.org)))
  # w.RPS <- which(grepl(pattern = "^RPS", x = rownames(obj.org)))
  # w.RPL <- which(grepl(pattern = "^RPL", x = rownames(obj.org)))
  # w.total.filter <- which(grepl(pattern = "^MT-|^ERCC-|^RPS|^RPL", x = rownames(obj.org)))
  # 
  # # mitocondrial genes
  # length(w.MT)
  # # ERCC RNA Spike-In Mix
  # length(w.ERCC)
  # #Gene group: S ribosomal proteins (RPS)
  # length(w.RPS)
  # #Gene group: L ribosomal proteins (RPL)
  # length(w.RPL)
  # 
  # #filter all "^MT-|^ERCC-|^RPS|^RPL"
  # length(w.total.filter)
  ...
  for(k in 1:N.f){
    ...
    k.results = comp_N.features(dataName, nfeatures=nfeatures.list[k], obj, real.subtype, expGroups=expGroups)
...
comp_N.features = function(dataName, nfeatures, obj, real.subtype, expGroups=expGroups){
  ...
  EGR.mat.G4[i,j] = EGR$G4
  EGR.mat.G5[i,j] = EGR$G5
  print("method2.scT v2.HDG...ARI Done!")
  ...
  EGR.mat.G4[i,j] = EGR$G4
  EGR.mat.G5[i,j] = EGR$G5
  print("method3.scT v2.HEG.cpm...ARI Done!")
  ...
  #obj.scTv2.DUBStepR <- generate_clustering_results.Vresol.obj_features(obj.name = obj.scTv2.DUBStepR, subtype = real.subtype,  method="DUBStepR", nfeatures = nfeatures, dist=NULL)
  ...
  #obj.logN.DUBStepR <- generate_clustering_results.Vresol.obj_features(obj.name = obj.logN.DUBStepR, subtype = real.subtype,  method="DUBStepR", nfeatures = nfeatures, dist=NULL)
  obj.logN.DUBStepR <- generate_clustering_results.Vresol.obj_features_DUBStepR(dataName, model.name="logN", obj.name = obj.logN.DUBStepR, subtype = real.subtype,  method="DUBStepR", nfeatures = nfeatures, dist=NULL, filterN="filter0.05")
...
generate_clustering_results.Vresol.obj_features_DUBStepR <- function(dataName, model.name, obj.name, subtype, method, nfeatures, dist=NULL, filterN="filter0.05"){
  #We find that setting this parameter between 0.6-1.2 typically returns good results for single cell datasets of around 3K cells. 
  # ari.clus.resol = 1.5 # >1.2 
  obj = obj.name
  clus.minNum=length(table(subtype))
  ...
  message(paste("nF:", length(DUBStepR.nF.ordered), "-",   length(features)), "- #NA?",  length(which(is.na(features))))
